...
3. Builds (`npm run build`), runs `npx wrangler pages deploy ./dist --project-name=cyber-pujangga --branch=main --commit-dirty=true`, then deletes all prior deployments except the newest via `cf-pages-cleanup.py`.
