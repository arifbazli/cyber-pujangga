...
import { en, type Strings } from "./lang/en"
...
const sectionMap = {
  ms: { esei: "essays", jurnal: "journal", sajak: "poems", tentang: "about" } as Record<string, string>,
  en: { essays: "esei", journal: "jurnal", poems: "sajak", about: "tentang" } as Record<string, string>,
}
...
async function buildSlugTranslationMap() {
  ...
  for (const collection of collections) {
    ...
    const byKey = { ms: new Map<string, CollectionEntry<typeof collection>>(), en: new Map<string, CollectionEntry<typeof collection>>() }
    for (const e of all) {
      ...
      if (!d) continue
      const formKey = collection === "poems" && (e.data as { form?: string }).form ? `:${(e.data as { form?: string }).form}` : ""
...
function slugOf(entry: CollectionEntry<"essays" | "journal" | "poems" | "pages">): string {
  return entry.id.split("/").slice(1).join("/").replace(/\.(md|mdx)$/, "")
}
