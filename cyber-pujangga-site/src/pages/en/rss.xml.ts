...
export async function GET(context: APIContext) {
  ...
  const items = [
    ...filterByLocale(essays,  "en").map((e) => ({
      ...
      link: `/essays/${e.id.split("/").slice(1).join("/").replace(/\.(md|mdx)$/, "")}`,
    })),
    ...filterByLocale(journal, "en", "date").map((j) => ({
      title: j.data.title,
      pubDate: j.data.pubDate,
      description: j.data.description ?? "",
      link: `/journal/${j.id.split("/").slice(1).join("/").replace(/\.(md|mdx)$/, "")}`,
    })),
    ...filterByLocale(poems,   "en").map((p) => ({
      title: p.data.title,
      pubDate: p.data.pubDate,
      description: p.data.description ?? `${p.data.form} poem`,
      link: `/poems/${p.id.split("/").slice(1).join("/").replace(/\.(md|mdx)$/, "")}`,
    })),
