...
export const expressiveCodeOptions: AstroExpressiveCodeOptions = {
  styleOverrides: {
    ...
    frames: {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      editorTabBarBackground: ({ theme }: any) => (theme.type === "light" ? "#ebe3cd" : "#15120e"),
      frameBoxShadowCssValue: "none",
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      terminalBackground: ({ theme }: any) => (theme.type === "light" ? "#f0e9d6" : "#1a1715"),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  themeCssSelector(theme: any, { styleVariants }: any) {
    if (styleVariants.length >= 2) {
      ...
      const altTheme = styleVariants.find((v: any) => v.theme.type !== baseTheme?.type)?.theme;
